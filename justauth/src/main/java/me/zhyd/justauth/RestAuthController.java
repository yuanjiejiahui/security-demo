...
public class RestAuthController {
    ...
    public Response renderAuth(@PathVariable("source") String source) {
        log.info("进入render：" + source);
        AuthRequest authRequest = getAuthRequest(source);
        String authorizeUrl = authRequest.authorize(AuthStateUtils.createState());
        log.info(authorizeUrl);
//        response.sendRedirect(authorizeUrl); https跨域问题
        return new Response(200,"正在跳转至第三方登录页面",authorizeUrl);
    }
    ...
    public Response login(@PathVariable("source") String source, AuthCallback callback, HttpServletResponse resp) throws IOException {
        ...
        if (response.ok()) {
            ...
            String jwt = "asdkahdkjashdka";
            map.put("token",jwt);
            map.put("info",response);
            resp.sendRedirect("http://127.0.0.1:8443?token="+jwt);
        }
        return new Response(200,"跳转到首页",map);
    }
...
//                        .redirectUri("http://localhost:9001/oauth/baidu/callback")
            ...
            case "gitee":
                authRequest = new AuthGiteeRequest(AuthConfig.builder()
                        .clientId("e6bdeef064190404c0af8e81ab330e45ea9adc49a161e3bf0cdbdb97a7ee001a")
                        .clientSecret("ee9abd775c9479987f04cae15fd0c104fb761512ba5294d667eab0cc8496a315")
